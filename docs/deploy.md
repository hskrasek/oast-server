...
7. One-time: broaden the `oast-tofu` Cloudflare token to include **API Tokens Write**
...
8. GitHub repo variables: `AWS_DEPLOY_ROLE_ARN`, `AWS_REGION`, `ECR_REPOSITORY`.
9. Steady state: merge to main → GHA tests → image → deploy. Publications changes are code, so
   publishing a review deploys like any commit.
10. Rollback: task definition revisions do **not** pin images in this setup (every revision's
