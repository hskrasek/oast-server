...
**resource-relationships** (Resource Relationships), assigning each a severity and a
confidence.
...
- blocker: will force a breaking change, corrupt data, or break clients later — e.g. deep
  nesting that bakes ownership into URLs and breaks when ownership moves.
- should-fix: real design debt — a model that forces client-side N+1, or a traversal the
  client needs that cannot be expressed.
- consider: genuine judgment call — expand/include syntax inconsistency, context-dependent
  choices.
...
finding's `dimension` to `resource-relationships`. Populate every required field. A
split / blocker (two panelists say a nesting choice forces a v2, one disagrees) is the
most valuable finding you can produce.
