<x-app-layout title="Access tokens"><h1 class="o-headline">Access tokens</h1>
...
<form method="POST" action="{{ route('app.settings.tokens.destroy', $token) }}">@csrf @method('DELETE')<button data-confirm="Revoke this token?" type="submit">Revoke</button></form></div>@endforeach</x-app-layout>
